...
module f32_convert
        ...
        contains
                ...
                subroutine build_f32_lookup_table
                        ...
                        !f32_lookup_table(v) = fp16_to_real32(v)
                        end do
...
contains 
        ...
        function p_mlp(xb,row,l, w) result(hb)
                ...
                !temp = fp16_to_real32(w%w1(:,row,l))
                hb = dot_product(xb,temp)
                temp = v_half_to_float_c(w%w3(:,row,l))
                !temp = fp16_to_real32(w%w3(:,row,l))
...
!
        ...
        function v_dot_product(a,b) result(c)
                real(kind=wp) :: a(:)
                integer(2) ::  b(:)
                real(kind=wp) :: c, r
                integer :: i
      ...
      ! see https://jaykmody.com/blog/gpt-from-scratch/ and ref therin for how attention works
        function transformer(token, pos, p, s, w) result(logits)
                ...
                do l = 1,p%n_layers
                        ...
                        !temp = v_half_to_float_lookup(w%wq(:,row,l))
                        q(ix) = dot_product(xb,v_half_to_float_c(w%wq(:,ix,l)))
                        k(ix) = dot_product(xb,v_half_to_float_c(w%wk(:,ix,l)))

                        v(ix) = dot_product(xb,v_half_to_float_c(w%wv(:,ix,l)))
                        !q(ix) = dot_product(xb,fp16_to_real32(w%wq(:,ix,l)))
                        !k(ix) = dot_product(xb,fp16_to_real32(w%wk(:,ix,l)))

                        !v(ix) = dot_product(xb,fp16_to_real32(w%wv(:,ix,l)))

                        !q(ix) = v_dot_product(xb,w%wq(:,ix,l))
                        !k(ix) = v_dot_product(xb,w%wk(:,ix,l))

                        !v(ix) = v_dot_product(xb,w%wv(:,ix,l))
                        ...
                        !x = x + matmul(xb, fp16_to_real32(w%wo(:,:,l)))
                        ...
                        !temp2 = fp16_to_real32(w%w2(:,ix,l))
                ...
                if (shared_weights) then
                        ...
                        logits = vm_matmul(x,fp16_to_real32(w%token_embedding_table)) 
                else
                        !$OMP PARALLEL DO PRIVATE (ix, temp)
                        do ix = 1,p%vocab_size
                                !temp = v_half_to_float_lookup(w%wcls(:,ix))
                                temp = fp16_to_real32(w%wcls(:,ix))
  ...
  pure function real4_to_bf16(value) result(bf16_value)
    real(wp), intent(in) :: value
    integer(2) :: bf16_value
    integer(wp) :: int_value
    ! Convert real to integer bits
    int_value = transfer(value, int_value)
    ! Shift right to drop the lower 16 bits and convert to integer(2)
    bf16_value = int_value / 2**16
  end function real4_to_bf16

    pure elemental function bf16_to_real4(bf16_value) result(value)
    integer(2), intent(in) :: bf16_value
    real(wp) :: value
    integer(wp) :: int_value
    ! Convert BF16 to integer bits and shift left to make it 32 bit
    int_value = bf16_value * 2**16
    ! Transfer the bits back to real
    value = transfer(int_value, value)
  end function bf16_to_real4
