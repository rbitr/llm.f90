...
contains
        ...
        type(TransformerWeights16), intent(out) :: w
        ...
        character(:), dimension(:), allocatable :: merge_tokens
        integer(4), allocatable :: merge_token_lengths(:)
        integer(4) :: magic, version
        integer(8) :: kv_pairs
        !class(*), allocatable :: demo
        integer :: max_len = 64
        integer :: i, j, val_type,file_pos,  alignment, deficit, id 
        integer(4) :: num_layers, emb_length, context_length, head_count, ffn_length, kv_heads, vocab_size
        integer(4) :: rope_dim_count
        ...
        open(UNIT=5, FILE=filename, FORM="UNFORMATTED",&
                ...
                do i = 1,kv_pairs
                        ...
                        else if (keys(i) .eq. "phi2.block_count") then
                                num_layers = values(i)%i32 !assume it's int(4)
                        else if (keys(i) .eq. "phi2.embedding_length") then
                                emb_length = values(i)%i32
                        else if (keys(i) .eq. "phi2.attention.head_count") then
                                head_count = values(i)%i32
                        else if (keys(i) .eq. "phi2.context_length") then
                                context_length = values(i)%i32
                        else if (keys(i) .eq. "tokenizer.ggml.tokens") then
                                vocab_size = (size(values(i)%a))
                        else if (keys(i) .eq. "phi2.attention.head_count_kv") then
                                kv_heads = values(i)%i32
                        else if (keys(i) .eq. "phi2.feed_forward_length") then
                                ffn_length = values(i)%i32
                        else if (keys(i) .eq. "phi2.rope.dimension_count") then
                                rope_dim_count = values(i)%i32
                        end if
                ...
                if (verbose) then 
                        ...
                        print *, "Rope dims: ", rope_dim_count
                ...
                c%rope_dim_count = rope_dim_count
                ...
                w%token_embedding_table = temp_gt%f162d
                ...
                allocate(w%rms_att_bias(emb_length,num_layers))
                do i = 1,num_layers
                        write(tempstr,"(A,I0,A)") "blk.", i-1, ".attn_norm.bias"
                        t0 = tensor_by_name(tempstr)
                        temp_gt = read_layer(5,t0,file_pos)
                        ! should be f32
                        !call write_tensor(8,temp_gt)
                        w%rms_att_bias(:,i) = temp_gt%f321d
                end do
                if (verbose) then
                        print *, "loaded rms att biases:", size(w%rms_att_bias)
                end if
                ...
                if (verbose) then
                        print *, "loaded bo weights:", size(w%bo)
                end if



                allocate(w%wup(emb_length,ffn_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%wup(:,:,i) = temp_gt%f162d
                end do

                if (verbose) then
                        print *, "loaded up weights:", size(w%wup)
                end if

                allocate(w%bup(ffn_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%bup(:,i) = temp_gt%f321d
                end do

                if (verbose) then
                        print *, "loaded up biasess:", size(w%bup)
                end if

                allocate(w%wdown(ffn_length,emb_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%wdown(:,:,i) = temp_gt%f162d
                end do

                if (verbose) then
                        print *, "loaded up weights:", size(w%wdown)
                end if

                allocate(w%bdown(emb_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%bdown(:,i) = temp_gt%f321d
                end do

                if (verbose) then
                        print *, "loaded down bias:", size(w%bdown)
                end if
                ...
                t0 = tensor_by_name("output_norm.bias")
                temp_gt = read_layer(5,t0,file_pos)
                ! f32
                !call write_tensor(8,temp_gt)
                w%rms_final_bias = temp_gt%f321d

                if (verbose) then
                        print *, "loaded output norm weights:", size(w%rms_final_bias)
                end if
                ...
                ! f16
                !call write_tensor(8,temp_gt)
                w%wcls = temp_gt%f162d

                if (verbose) then
                        print *, "loaded classifier weights:", size(w%wcls)
                end if

                t0 = tensor_by_name("output.bias")
                temp_gt = read_layer(5,t0,file_pos)
                ! f16
                !call write_tensor(8,temp_gt)
                w%bcls = temp_gt%f321d

                if (verbose) then
                        print *, "loaded classifier weights:", size(w%bcls)
                end if
        ...
        if (.true.) then
                ...
                do i = 1,kv_pairs
                        ...
                        else if (tempstr .eq. "tokenizer.ggml.merges") then
                                if (verbose) then
                                print *, "loading merges"
                                end if
                                ! allocate
                                read(5) temp_int, tmp_vocab_size
                                allocate(character(len=max_len) ::  merge_tokens(tmp_vocab_size))
                                allocate(merge_token_lengths(tmp_vocab_size))
                                do j=1,int(tmp_vocab_size,4)
                                        read(5) strlen
                                        allocate(character(strlen) :: loaded_str)
                                        read(5) loaded_str
                                        associate (split => index(loaded_str, " "))
                                        merge_tokens(j) = loaded_str(1:split-1) // loaded_str(split+1:)
                                        end associate        
                                        merge_token_lengths(j) = len(trim(merge_tokens(j)))
                                        
                                        !merge_tokens(j) = loaded_str
                                        deallocate(loaded_str)
                                end do
                                if (verbose) then
                                write (*,"(A,I0,A)") "found ", size(merge_tokens), " merges"
                                !do j=1,10
                                !print *, merge_tokens(j), merge_token_lengths(j)
                                !end do
                                end if


                        else if (tempstr .eq. "tokenizer.ggml.scores") then
                                print *, "Tokenize scores not supported"
                                stop
                ...
                tmp_vocab_size = size(vocab) 
                allocate(scores(size(vocab)))

                scores = 0
                associate (max_score => size(merge_tokens))
                do j=1,max_score
                ! find index in vocab of current merge
                id = -1 
                do i=1,size(vocab)
                if (vocab(i) == merge_tokens(j)) then
                        id = i
                        exit
                end if
                end do
                ...
                if (id == -1) then
                        print *, "token not found", merge_tokens(j)
                        stop
                end if

                scores(id) = max_score - j
       ...
       ! end if
        ...
        recursive function read_val(handle, val_type) result (val)
                ...
                else if (val_type .eq. 7) then
                        read(handle) bool_temp
                        if (verbose) then
                        print *, "Encountered boolean variable that was not read", bool_temp
                        end if
                else 
