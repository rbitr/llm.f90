...
contains 
        ...
        function transformer(token, pos, s, w) result(logits)
                ...
                do l = 1,n_layers
                        ...
                        !        q0 = q(i+j-1)
                        !        q1 = q(i+j+15)
                        !        q(i+j-1) = q0 * fcr - q1 * fci
                        !        q(i+j+15) = q0 * fci + q1 * fcr
                        !        k0 = k(i+j-1)
                        !        k1 = k(i+j+15)
                        !        k(i+j-1) = k0 * fcr - k1 * fci
                        !        k(i+j+15) = k0 * fci + k1 * fcr
                        ! 
                        !end do
                        !end do
                        ...
                        do i=1,head_size,2
                               ...
                               !head_dim = mod(i-1,head_size)
                                ...
                                fcr = cos(rval)
                                fci = sin(rval)
                                q0 = q((h-1)*head_size+i)
                                q1 = q((h-1)*head_size+i+1)
                                q((h-1)*head_size+i) = q0 * fcr - q1 * fci
                                q((h-1)*head_size+i+1) = q0 * fci + q1 * fcr
                                !if (i<kv_head_size) then
                                k0 = k((h-1)*head_size+i)
                                k1 = k((h-1)*head_size+i+1)
                                k((h-1)*head_size+i) = k0 * fcr - k1 * fci
                                k((h-1)*head_size+i+1) = k0 * fci + k1 * fcr
                                !end if
